(() => {
  ...
  function cfg(input = state.config) {
    ...
    merged.antiDuckSustain = Boolean(merged.antiDuckSustain);
    merged.rawMicConstraintLock = Boolean(merged.rawMicConstraintLock);
    merged.sustainTargetDb = clamp(merged.sustainTargetDb, -24, -0.1);
  ...
  function applyPipeline(pipeline, inputConfig = state.config) {
    ...
    pipeline.sustain = {
      enabled: c.antiDuckSustain,
      targetLinear: dbToLinear(c.sustainTargetDb),
      maxGain: c.sustainMaxGain
    };
  ...
  function ensureSustainLoop(pipeline) {
    if (!pipeline || pipeline.sustainTimer) return;
    const { nodes, ctx } = pipeline;
    if (!nodes?.sustainMeter || !nodes?.sustainGain) return;
    pipeline.sustainTimer = setInterval(() => {
      if (!pipeline.sustain?.enabled || ctx?.state === "closed") return;
      try {
        nodes.sustainMeter.getFloatTimeDomainData(nodes.sustainBuffer);
        let sum = 0;
        for (let i = 0; i < nodes.sustainBuffer.length; i++) {
          const v = nodes.sustainBuffer[i] || 0;
          sum += v * v;
        }
        const rms = Math.sqrt(sum / nodes.sustainBuffer.length);
        const current = Math.max(0.0005, rms);
        const desired = clamp((pipeline.sustain.targetLinear || 0.8) / current, 1, pipeline.sustain.maxGain || 64);
        setParam(nodes.sustainGain.gain, desired, ctx);
      } catch (_) {}
    }, 80);
  }
  ...
  function build(stream, inputConfig) {
    ...
    const sustainMeter = ctx.createAnalyser();
    sustainMeter.fftSize = 2048;
    ...
    loudness.connect(sustainGain);
    sustainGain.connect(sustainMeter);
    sustainGain.connect(gain);
    gain.connect(saturator);
    saturator.connect(limiter);
    limiter.connect(dst);

    const pipeline = { ctx, nodes: { low, pres, high, comp1, loudness, sustainGain, sustainMeter, gain, saturator, limiter }, sustainBuffer: new Float32Array(sustainMeter.fftSize) };
  ...
  function normalizeConstraints(constraints) {
    ...
    if (typeof next.audio === "object") {
      ...
      if (!activeCfg.rawMicConstraintLock) {
        const originalAudio = constraints && typeof constraints === "object" ? constraints.audio : undefined;
        if (typeof originalAudio === "object") next.audio = { ...next.audio, ...originalAudio };
      }
    }
    return next;
  }
